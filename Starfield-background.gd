...
@onready var rainbow_stars = preload("res://rainbow-stars.tres")
...
func go_rainbow():
	...
	# We can't change the amount without resetting, so instead we enable an extra layer.
	$GPUParticles2DMany.process_material = rainbow_stars
	$GPUParticles2DTurbo.process_material = rainbow_stars
